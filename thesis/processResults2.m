...
figure;
hold on;
title(charttitle, 'FontWeight', 'bold', 'FontSize', 14);
area_handle = 0;
X = proc_results(:, 1);
...
if strcmp(axismode, 'linear') == 1
	...
	xlabel ('elite influence factor','FontSize', 14);
	ylabel ('voters (%)','FontSize', 14);
	color = [.8 .8 1];
...
elseif strcmp(axismode, 'log') == 1	
	...
	xlabel ('elite size (n^x)','FontSize', 14);
	ylabel ('voters (n^x)','FontSize', 14);
	color = [.8 .8 1];
	axis([0 1 0 1]);
	plot(X, X, 'LineWidth', 2, 'Color', color);
